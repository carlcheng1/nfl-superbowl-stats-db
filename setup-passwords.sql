...
BEGIN
  ...
  IF username NOT IN (SELECT users.username FROM user_info AS users) THEN 
    RETURN 0;
  END IF;

  SELECT users.salt, users.password_hash INTO salt, password_hash 
  FROM user_info AS users WHERE users.username = username;
...
CALL sp_add_user('sbadmin', 'password');
CALL sp_add_user('carl', 'letmein');
